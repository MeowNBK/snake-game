...
#include <fstream>
#include <iostream>
#include "common.h" // Sử dụng các hằng số và cấu trúc chung
#include "libs/colors.h"

// void render_text(const std::string& text, int x, int y, struct SDL_Color color, struct _TTF_Font* font, bool is_centered);
...
void check_achievements(
    ...
    Uint32 game_time_ms, Uint32& mirror_match_start_time
) {
    // --- Thành tích cho Người chơi ---
    if (player.is_alive) {
    ...
    if (ai.is_alive) {
        ...
        if (play_mode == PlayMode::AI_ONLY && (game_time_ms / 1000 >= 180)) unlock("AI_SURVIVE_3_MIN");
    }
    ...
    //         SDL_Rect bg_rect = {(WINDOW_W - text_w - 20) / 2, WINDOW_H - 60, text_w + 20, text_h + 20};
    //         SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND); // Bật chế độ trong suốt
    //         SDL_SetRenderDrawColor(renderer, 20, 20, 20, 200); // Màu đen mờ
    //         SDL_RenderFillRect(renderer, &bg_rect);

    //         // Vẽ chữ đè lên trên
    //         SDL_Rect dst_rect = {(WINDOW_W - text_w) / 2, WINDOW_H - 50, text_w, text_h};
    ...
    //         notification_start_time = 0; // Reset timer cho thông báo tiếp theo
    //     }
    // }
    ...
    //     // Render tiêu đề
    //     render_text_helper(renderer, font, "PHONG TRUYEN THONG", WINDOW_W / 2, 50, Colors::NeonYellow, true);
    ...
    //             render_text_helper(renderer, font, ach.name, 50, y_pos, Colors::VaporNeonGreen);
    //             render_text_helper(renderer, font, ach.description, 50, y_pos + 25, Colors::White);
    //         } else {
    //             // Hiển thị mờ và ẩn mô tả nếu chưa mở khóa
    //             render_text_helper(renderer, font, ach.name, 50, y_pos, Colors::Gray50);
    //             render_text_helper(renderer, font, "????????????????", 50, y_pos + 25, Colors::DarkGray);
    //         }
    //         y_pos += 60;
    //     }
    //     render_text_helper(renderer, font, "Nhan ESC de quay lai Menu", WINDOW_W / 2, WINDOW_H - 40, Colors::Cyan, true);
    // }

    void render_list_screen(SDL_Renderer* renderer, TTF_Font* font_title, TTF_Font* font_text);

    // Thêm một hàm helper để render text cho gọn (có thể đặt ở file riêng hoặc trong game.cpp)
    // void render_text_helper(SDL_Renderer* r, TTF_Font* f, const std::string& text, int x, int y, SDL_Color c, bool centered = false) {
