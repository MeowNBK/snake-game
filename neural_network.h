...
public:
    ...
    void backward_pass(const std::vector<double>& target, bool accumulate_gradients = false);
    ...
    void load_from_vectors(const std::vector<double>& flat_weights, const std::vector<double>& flat_biases);
...
inline NeuralNetwork::NeuralNetwork(const std::vector<int>& topology) : topology(topology) {
    std::mt19937 gen(std::random_device{}());
    for (size_t i = 0; i < topology.size() - 1; ++i) {
        int fan_in = topology[i]; int fan_out = topology[i + 1];
        std::uniform_real_distribution<double> dist_xavier(-sqrt(6.0 / (fan_in + fan_out)), sqrt(6.0 / (fan_in + fan_out)));
        weights.push_back(std::vector<std::vector<double>>(fan_out, std::vector<double>(fan_in)));
        for (auto& row : weights.back()) for (auto& val : row) val = dist_xavier(gen);
        biases.push_back(std::vector<double>(fan_out, 0.1));
        m_dw.push_back(std::vector<std::vector<double>>(fan_out, std::vector<double>(fan_in, 0.0)));
        v_dw.push_back(std::vector<std::vector<double>>(fan_out, std::vector<double>(fan_in, 0.0)));
        m_db.push_back(std::vector<double>(fan_out, 0.0));
        v_db.push_back(std::vector<double>(fan_out, 0.0));
        gradients_w.push_back(std::vector<std::vector<double>>(fan_out, std::vector<double>(fan_in, 0.0)));
        gradients_b.push_back(std::vector<double>(fan_out, 0.0));
    }
}


inline std::vector<double> NeuralNetwork::forward_pass(const std::vector<double>& input) {
    if (input.size() != (size_t)topology[0]) throw std::runtime_error("Lỗi: Kích thước input không khớp!");
    ...
    for (size_t i = 0; i < weights.size(); ++i) {
        ...
        for (size_t j = 0; j < n_neurons; ++j) {
            double z = biases[i][j];
            for (size_t k = 0; k < topology[i]; ++k) z += weights[i][j][k] * activations[i][k];
            z_values[i + 1][j] = z;
            Activation act_type = (i == weights.size() - 1) ? Activation::LINEAR : Activation::RELU;
...
inline void NeuralNetwork::clear_gradients() {
    for (auto& layer : gradients_w) for (auto& neuron : layer) std::fill(neuron.begin(), neuron.end(), 0.0);
    for (auto& layer : gradients_b) std::fill(layer.begin(), layer.end(), 0.0);
}

inline void NeuralNetwork::backward_pass(const std::vector<double>& target, bool accumulate_gradients) {
    if (!accumulate_gradients) clear_gradients();
    
    size_t output_layer_idx = topology.size() - 1;
    if(target.size() != topology[output_layer_idx]) throw std::runtime_error("Lỗi: Kích thước target không khớp!");
    ...
    for (size_t i = 0; i < topology[output_layer_idx]; ++i) {
        ...
        deltas[output_layer_idx][i] = error * apply_activation_derivative(z_values[output_layer_idx][i], Activation::LINEAR);
    }
    ...
    for (size_t i = output_layer_idx - 1; i > 0; --i) {
        ...
        for (size_t j = 0; j < topology[i]; ++j) {
            ...
            deltas[i][j] = error_propagated * apply_activation_derivative(z_values[i][j], Activation::RELU);
        }
    }

    // Giai đoạn 3: Dùng các Delta đã tính để tính Gradient cho tất cả các weight và bias
    for (size_t i = 0; i < weights.size(); ++i) {
...
inline void NeuralNetwork::update_parameters(double learning_rate, int batch_size) {
...
inline void NeuralNetwork::save_weights(const std::string& filename) {
    ...
    if (!file) return;
    for (const auto& layer : weights) for (const auto& neuron : layer) file.write(reinterpret_cast<const char*>(neuron.data()), neuron.size() * sizeof(double));
    for (const auto& layer : biases) file.write(reinterpret_cast<const char*>(layer.data()), layer.size() * sizeof(double));
}

inline void NeuralNetwork::load_weights(const std::string& filename) {
    std::ifstream file(filename, std::ios::binary);
    if (!file) { std::cerr << "Không tìm thấy file brain, dùng weights ngẫu nhiên.\n"; return; }
    for (auto& layer : weights) for (auto& neuron : layer) file.read(reinterpret_cast<char*>(neuron.data()), neuron.size() * sizeof(double));
    for (auto& layer : biases) file.read(reinterpret_cast<char*>(layer.data()), layer.size() * sizeof(double));
...
inline double NeuralNetwork::apply_activation(double x, Activation type) {
    switch (type) {
        ...
        case Activation::SIGMOID: return 1.0 / (1.0 + exp(-x));
        case Activation::LINEAR:
        default:                  return x;
    }
}
inline double NeuralNetwork::apply_activation_derivative(double z, Activation type) {
    switch (type) {
        case Activation::RELU:    return (z > 0) ? 1.0 : 0.0;
...
inline void NeuralNetwork::load_from_vectors(const std::vector<double>& flat_weights, const std::vector<double>& flat_biases) {
    size_t weight_idx = 0;
...
// Hàm Đột biến: Thay đổi ngẫu nhiên một vài gen
inline void NeuralNetwork::mutate(double mutation_rate, double mutation_strength) {
    std::mt19937 gen(std::random_device{}());
    std::uniform_real_distribution<double> dist(-mutation_strength, mutation_strength);
